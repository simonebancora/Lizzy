oValidation cases
================

We can identify some cases that have a reference solution (either analytical or established) and solve the same with Lizzy to compare results.

Channel flow experiment
-----------------------

The channel flow experiment, as described in the well-known work by Weitzenbock, has an analytical solution. For a constant inlet pressure and a perfectly one-dimensional flow, the time of arrival of the flow front at a distance L from the inlet is expressed as:

.. math::

   t = \frac{\phi \mu L^2}{2 \Delta p \mathbf{K}}

Case definition
~~~~~~~~~~~~~~~

To conduct the channel flow experiment, we will use the following values:

- :math:`\phi` = 0.5
- :math:`\mu = 0.1` Pa·s
- :math:`\Delta p =` 1.0E+05 Pa
- :math:`\mathbf{K}` = 1.0E-10 :math:`\cdot I` m²

By plugging the values in Eq. (XXXX), and considering a distance L = 1m, we obtain a theoretical arrival time of: :math:`t_{ref}` = **2500s**. This shall be considered as the reference solution.

Lizzy solution
~~~~~~~~~~~~~~

The simulated scenario is the case that was solved in the :ref:`Channel flow experiment <channel_flow>` tutorial, with dimensions L = 1 m, W = 0.5 m.

.. image:: ../../images/rect_mesh_1024elem_fill.png
   :height: 300px
   :align: center

Running the simulation with increasing number of elements we obtain the following results:

.. list-table::
   :header-rows: 1
   :align: center

   * - Number of elements
     - Fill time [s]
     - % Error
   * - 16
     - 2530.1
     - 1.20
   * - 64
     - 2510.5
     - 0.42
   * - 256
     - 2503.2
     - 0.13
   * - 1024
     - 2500.7
     - 0.03
   * - 4096
     - 2500.1
     - 0.005

.. image:: ../../images/validation_rect_err.png
   :width: 450px
   :align: center

As the number of elements increases, the fill time of the part approaches the theoretical solution and the error decreases.

Radial flow experiment
----------------------

For this validation we simulate a radial infusion experiment with an isotropic material. The analytical solution for time of arrival of the flow front at a radial distance :math:`R` from the center is:
